...
def main():
	...
	with socket.socket( socket.AF_INET,
		...
		box_header = struct.Struct(f'>{uid_len}sI')
		data_len = mtu - pkt_header.size

		uid_mask_apply = lambda uid: (
			(int.from_bytes(uid, 'big') & ~uid_mask)
		...
		def data_send(uid, buff):
			...
			while buff:
				...
				frame = pkt_header.pack(uid_mark, frame_n) + frame
